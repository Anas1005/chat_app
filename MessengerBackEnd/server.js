...
io.on("connection", async (socket) => {
  ...
  if (user_id != null && Boolean(user_id)) {
    try {
      ...
      socket.emit("Checking","It is Checked");
  ...
  //   console.log("Checked Emitted From Client",data);
  // })

  // Socket event listener for friend request
  ...
  socket.on("accept_request", async (data) => {
    ...
    try {
      ...
      await Promise.all([
        User.findByIdAndUpdate(sender._id, { $push: { friends: requestDoc.recipient } }),
        User.findByIdAndUpdate(receiver._id, { $push: { friends: requestDoc.sender } }),
      ]);
